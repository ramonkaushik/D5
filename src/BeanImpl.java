...
public class BeanImpl implements Bean {
	...
	private int totalRights; // number of times the bean is going to go right. Synonomous with skill level
	private int rightsRemaining;
	...
	BeanImpl(int slotCount, boolean isLuck, Random rand) {
		...
		this.isLuck = isLuck;
		this.rand = rand;
		this.xpos = 0; // bean starts at position 0
		/*
		...
		*/
		...
		this.skillStdDev = (double) Math.sqrt(slotCount * 0.5 * 0.5);
		this.totalRights = (int) Math.round(rand.nextGaussian() * this.skillStdDev +  this.skillAverage);
		this.rightsRemaining = this.totalRights;
	...
	public void choose() {
		// TODO: Implement
		if (this.isLuck) {
			// if its 1, its moving right, and therefore increment pos
			if (this.rand.nextInt(2) == 1) {
				++this.xpos;
			}
		//if its not luck, its skill which is based on the var right turn remaining
		} else if (this.rightsRemaining > 0) {
			++this.xpos;
			--this.rightsRemaining;
		}
